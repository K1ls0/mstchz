...
pub fn RenderState(comptime config: LibraryConfiguration) type {
    return struct {
        pub const Self = @This();

        const Tokens = tokens.Tokens(config);
        ...
        pub fn render(self: *Self, writer: anytype) (RenderError || mem.Allocator.Error || @TypeOf(writer).Error)!void {
                ...
                switch (ctoken) {
                    ...
                    .tag => |t| switch (t.type) {
                        ...
                        .section_close => {
                            ...
                            const start_accessor = cscope.getStartAccessor(self.input) orelse return error.NoStartAccessor;
                            ...
                            switch (cscope.state) {
                                .block => _ = self.scopes.pop(t.body),
                                .iter => |*it| {
                        ...
                        .partial, .partial_dynamic => |p| {
                            ...
                            if (self.partials.*.get(partial_name)) |partial| {
                                ...
                                var state = try Self.initFromScopes(
                                    self.allocator,
                                    new_scopes,
                                    self.hash_ctx,
                                    self.partials,
